...
const CmpDropdown = () => {
  ...
  const onSearch = (txt: any) => {
    if (txt !== '') {
      let tmp = data.filter((item: any) => {
        return item.country.toLowerCase().indexOf(txt.toLowerCase()) > -1; //it will return those item who will match
      });
      setData(tmp);
    } else {
      setData(countries);
  ...
  return (
    <View style={styles.container}>
      ...
      {isClicked ? (
        <View style={styles.drowdownArea}>
          <TextInput
            ref={searchRef}
            placeholder="Search"
